...
# Patch by Hubert Garavel for mac64 (July 2019)
# On mac64 with XQuartz and an genuine Apple mouse, the <MouseWheel> event
# does not seem to exist, and will cause TK initialization to fail. This
# problem did not occur on the other architectures supported by CADP.
# To detect this situation, we test if <MouseWheel> is accepted by "bind"
# and we set a variable $CadpMouseWheelExists that will be used later in
# the other libraries Grid.tcl, HList.tcl, and TList.tcl

if {[catch {
      bind CadpDummyTag <MouseWheel> {
      }
   } CadpDummyError ] } {
   # bind failed: <MouseWheel> does not exist
   set CadpMouseWheelExists 0
} else {
   set CadpMouseWheelExists 1
}
