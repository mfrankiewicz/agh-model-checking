...
#
#  new code by Julien Henry (INRIA/VASY) Thu Jul 30 11:04:24 CEST 2009
#  to insert a folder icon or a file icon between the image and
#  the text of a node
#
# ----------------------------------------------------------------------------
#  Index of commands:
...
proc Tree::_draw_node { path node x0 y0 deltax deltay padx showlines } {
    ...
    ##########################################################################
    # Beginning of code modified by J. Henry - Thu Jul 30 11:04:24 CEST 2009
    #
    # The following, original fragment of code:
    #
    ...
    #
    # must be replaced by the following fragment of code:
    #
    ##########################################################################
    # Correction par Remi Herilier - Tue Mar 23 10:51:54 CET 2010
    #
    # Dans la ligne ci-dessous, il ne fallait pas ecrire
    #    [file isdirectory $node]
    # car $node n'est pas un nom de fichier, mais un identificateur interne
    # a BWidget::Tree qui sert d'indice dans des tables ; la plupart du temps
    # cet identificateur interne coincide avec le nom du fichier, sauf dans 
    # le cas particulier ou le nom du fichier comporte un caractere special
    # comme '!', '^', '&', # '|' ou "::" (dans ce cas, le resultat de [file
    # isdirectory $node] renvoie false meme si $node designe un repertoire)
    ##########################################################################

    if { [file isdirectory [_node_name_rev $path $node]] } {
	$path.c create image [expr {$x1+$padx-5}] $y0 \
	    -image  [Bitmap::get folder] \
	    -anchor w
    } else {
	$path.c create image [expr {$x1+$padx}] $y0 \
	    -image  [Bitmap::get file] \
	    -anchor w
    }

    # the text is +20 pixels in the right 
    $path.c create text [expr {$x1+$padx+20}] $y0 \
        -text   [Widget::getoption $path.$node -text] \
        -fill   [Widget::getoption $path.$node -fill] \
        -font   [Widget::getoption $path.$node -font] \
        -anchor w \
    ...
    ##########################################################################
    # End of code modified by J. Henry - Thu Jul 30 11:04:24 CEST 2009
    ##########################################################################
